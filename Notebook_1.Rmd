...
Now, let's add some understandable labels for our watershed treatments. To do so in a way that it is preserved for future plots, we need to modify our dataset "hjg". It turns out that we already have a column in our data set to do so, it's ws.f (watershed as a factor). You can call for any column in a dataset by using the $ sign.
